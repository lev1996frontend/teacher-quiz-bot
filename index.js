...
bot.action("certs", async (ctx) => {
  try {
    if (!isAllowed(ctx)) {
      // закрываем «часики» без алерта
      await ctx.answerCbQuery();

      // пишем обычное сообщение в текущий чат (внизу)
      await ctx.reply("Извини, сертификаты доступны только адресату 💌");
      return;
    }

    // доступ есть — просто закрываем «часики» и шлём сертификаты
    await ctx.answerCbQuery();
    await sendCertificates(ctx);
  } catch (e) {
    console.error("certs handler error:", e);
    // сообщаем о сбое обычным сообщением в текущий чат
    try {
